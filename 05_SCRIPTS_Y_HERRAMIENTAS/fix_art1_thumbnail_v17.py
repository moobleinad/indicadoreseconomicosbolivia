...
v17_path = os.path.join(root_dir, r"tema_optimizado\v17_theme_optimizado.xml")
sq_img_path = os.path.join(root_dir, "foto_articulo1_bth_cuadrada.jpg")
comp_art1_path = os.path.join(root_dir, "art1_sq_min.jpg")

# Compress square image to 260x260 px lightweight JPG (~14 KB)
im = Image.open(sq_img_path).convert("RGB")
im.thumbnail((260, 260))
im.save(comp_art1_path, "JPEG", quality=55)

with open(comp_art1_path, "rb") as f:
    b64_art1 = base64.b64encode(f.read()).decode("utf-8")

data_uri_art1 = f"data:image/jpeg;base64,{b64_art1}"
print(f"Compressed Art1 b64 len: {len(b64_art1)}")

# Rebuild v17 with super lightweight art 1 image
...
# SEO BLOCK
seo_block = """
  <!-- OPTIMIZACION SEO V17 MASTER DANIEL SIMONS -->
...
# HTML Frontpage Block V17 (WITH EMBEDDED LIGHTWEIGHT ART 1 THUMBNAIL)
frontpage_html_block = f"""
            <b:if cond='data:view.isHomepage'>

              <!-- SECCIÓN 1: ÚLTIMAS ENTRADAS DEL BLOG (CON MINIATURA REAL EMBEBIDA 100% GARANTIZADA) -->
              <section class="ds-section-block" id="sec-posts">
                ...
                <div class="ds-carousel-track" id="track-posts">
                  <a href="{URL_POST_1}" class="ds-card-box">
                    <div>
                      <img src="{data_uri_art1}" alt="De la idea escolar al proyecto ordenado" class="ds-card-thumb" />
...
with open(v17_path, "w", encoding="utf-8") as f:
    f.write(xml_content)

try:
    ET.parse(v17_path)
    print("SUCCESS: Master v17_theme_optimizado.xml updated and passed XML test!")
    file_size_kb = os.path.getsize(v17_path) / 1024
