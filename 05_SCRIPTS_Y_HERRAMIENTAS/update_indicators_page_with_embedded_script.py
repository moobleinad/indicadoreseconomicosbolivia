...
# Read full original dashboard HTML (45.5 KB)
with open(html_path, "r", encoding="utf-8") as f:
    full_html = f.read()

# Make sure the script tag is wrapped cleanly inside a type="text/javascript" block that Blogger API accepts
full_html = full_html.replace("<script>", '<script type="text/javascript">/*<![CDATA[*/')
full_html = full_html.replace("</script>", '/*]]>*/</script>')

print(f"PREPARED FULL ORIGINAL HTML DASHBOARD (Length: {len(full_html)} chars)!")
            ...
            for p in pages_data.get("items", []):
                if "indicadores" in p.get("title", "").lower() or "indicadores" in p.get("url", "").lower():
                    ...
                    with urllib.request.urlopen(req_up) as up_resp:
                        ...
                        print(f"SUCCESS: PUSHED FULL ORIGINAL DASHBOARD WITH CDATA JS TO '{p.get('title')}'! URL: {up_data.get('url')}")
