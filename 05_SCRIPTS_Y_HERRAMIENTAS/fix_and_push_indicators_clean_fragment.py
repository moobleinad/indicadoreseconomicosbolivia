...
# Combine into a clean HTML Fragment (Style + Body Content) suitable for Blogger Editor
fragment_html = f"{style_content}\n\n{body_content}"

print(f"EXTRACTED CLEAN HTML FRAGMENT (Length: {len(fragment_html)} chars)!")

# Refresh token & update live page on Blogger API
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            with urllib.request.urlopen(req_up) as up_resp:
                                ...
                                print(f"SUCCESS: PUSHED CLEAN HTML FRAGMENT TO '{p.get('title')}'! URL: {up_data.get('url')}")
                                break
