...
with urllib.request.urlopen(req_token) as resp:
    ...
    with urllib.request.urlopen(req_blogs) as b_resp:
        ...
        # Check posts status
        posts_url = f"https://www.googleapis.com/blogger/v3/blogs/{blog_id}/posts?fetchBodies=false"
        req_posts = urllib.request.Request(posts_url, headers=headers)
        with urllib.request.urlopen(req_posts) as p_resp:
            posts_data = json.loads(p_resp.read().decode("utf-8"))
            print("TOTAL POSTS FOUND:", len(posts_data.get("items", [])))
            for p in posts_data.get("items", []):
                print(f"- ID: {p.get('id')} | TITLE: {p.get('title')} | STATUS: {p.get('status')} | URL: {p.get('url')}")
