...
if "09.02_afiche_indicadores_economicos_cuadrado.webp" not in html_content:
    img_block = f'\n<div style="display:none !important; visibility:hidden !important;" class="ds-og-hidden-image"><img src="{cdn_url}" alt="Indicadores Económicos de Bolivia" width="1024" height="1024" /></div>\n'
    html_content += img_block
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            with urllib.request.urlopen(req_up) as up_resp:
                                ...
                                print(f"SUCCESS: UPDATED LIVE PAGE '{p.get('title')}' WITH CDN AFICHE IMAGE! URL: {up_data.get('url')}")
                                break
                        except Exception as ex:
                            print(f"Attempt {attempt+1} failed: {ex}. Retrying in 2 seconds...")
                            time.sleep(2)
