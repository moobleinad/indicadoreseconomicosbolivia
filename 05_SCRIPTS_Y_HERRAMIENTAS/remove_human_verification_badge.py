...
# 1. REMOVE FROM JSON DATA
with open(json_path, "r", encoding="utf-8") as f:
    datos = json.load(f)

for item in datos["indicadores"]:
    item.pop("verificacion_texto", None)
    item.pop("verificacion", None)

with open(json_path, "w", encoding="utf-8") as f:
    json.dump(datos, f, indent=2, ensure_ascii=False)

# 2. UPDATE HTML FILE TO REMOVE VERIFICATION BADGES ENTIRELY
with open(html_path, "r", encoding="utf-8") as f:
    html_content = f.read()

# Remove verification badge rendering from HTML
import re
html_content = re.sub(r'<div>\s*<span class="verification-badge [^"]*">[^<]*</span>\s*</div>', '', html_content)
html_content = re.sub(r'<div>\s*<span class="verification-badge">[^<]*</span>\s*</div>', '', html_content)
html_content = re.sub(r'\${item\.verificacion_texto}', '', html_content)

with open(html_path, "w", encoding="utf-8") as f:
    f.write(html_content)

print("REMOVED HUMAN VERIFICATION BADGE FROM LOCAL FILES!")

# 3. REFRESH TOKEN AND UPDATE LIVE ON BLOGGER
        ...
        with urllib.request.urlopen(req_pages) as p_resp:
            ...
            if target_page:
                ...
                with urllib.request.urlopen(req_up) as up_resp:
                    ...
                    print("PAGE UPDATED LIVE SUCCESSFULLY - BADGE REMOVED!")
