...
# Insert the image cleanly at the end of the page as a visible/proper image element just like Daniel uploaded
visible_img_html = f'''
<div style="text-align: center; margin-top: 40px; margin-bottom: 20px;" class="separator">
  <a href="{cdn_url}" style="margin-left: 1em; margin-right: 1em;">
    <img border="0" data-original-height="1024" data-original-width="1024" src="{cdn_url}" width="640" alt="Indicadores Económicos de Bolivia" />
  </a>
</div>
'''

if "09.02_afiche_indicadores_economicos_cuadrado.webp" not in html_content:
    html_content += visible_img_html

with open(html_path, "w", encoding="utf-8") as f:
    f.write(html_content)

print("LOCAL HTML UPDATED WITH DANIEL'S VISIBLE IMAGE TAG!")

# Now update live via API
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            with urllib.request.urlopen(req_up) as up_resp:
                                ...
                                print(f"SUCCESS: PUSHED DANIEL'S IMAGE LIVE TO '{p.get('title')}'! URL: {up_data.get('url')}")
                                break
